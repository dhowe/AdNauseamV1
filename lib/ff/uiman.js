...
var UIManager = require('sdk/core/heritage').Class({
    ...
    updateOnAdAttempt : function(ad) {
        ...
        if (this.menuIsOpen())
            this.menu.port.emit("set-current", { current: ad });
    ...
    updateMenu : function(update) { 
        ...
        if (this.menuIsOpen()) {
            ...
            //console.log('parser::current: '+(current ? current.id : -1));
    ...
    getMenuAds : function(parser, pageUrl) {
        ...
        var parser = require("./abpcomp").Component.parser,
            byField = require('./adnutil').AdnUtil.byField,
            pageMap = parser.pageMap[pageUrl],
            ads = (pageMap && pageMap.ads) || [];
                   
        //console.log('getMenuAds: '+ads.length);     
        
        if (ads.length) {
            
            ads.sort(byField('-foundTs')); // sort by found-time
            return { ads: ads, count: ads.length };
        }
        else {
            
            return { ads: this.recentAds(parser, byField), count: 0 };
        }
    },
