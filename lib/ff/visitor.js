...
var AdVisitor = require('sdk/core/heritage').Class({
	...
	/*
	 * Updates the visited ad, then checks for others (with )
	 */	
	updateOnVisit : function(visitedAd, time, opts) {
		
		// NOTE: may leave ads w' same image (but diff target) unvisited

		visitedAd.visitedTs = time; // don't reselect
		visitedAd.title = (time && opts) ? opts.title : 'Error';
		if (opts && opts.url) {
			visitedAd.resolvedUrl = opts.url;
			visitedAd.path.push(url);
		}

		var updated = [ visitedAd ], ads = this.parser.getAds();
			
		for (var i=0, j=ads.length; i< j; i++) {
			
			// match all ads with same target
		...
		for (var i=0, j=updated.length; i< j; i++) 
 			UIManager.updateOnAdVisited(updated[i]); // use map
	...
	restart : function() {
		...
		var script = "var s = '', elements = document.querySelectorAll('title'); "
  		...
  		Logger.log('Creating PageWorker: allowJavaScript='
  			+ this.allowJsOnPage + ' allowPlugins=false allowDialogs=false');
