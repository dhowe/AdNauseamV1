...
var AbpComponent = require('sdk/core/heritage').Class({
    ...
    processNode : function(wnd, node, type, loc, collapse) { 
        ...
        var parser, parentUrl, abpResult;
        ...
        try {
            ...
            else if (parser.ignoreRE.test(parentUrl)) { // plus vault/menu (TODO:)
            ...
            if (!Options.get('enabled')) return abpResult;
...
//              
                ...
                if (0 && !/ima?ge?/i.test(node.tagName) && node.tagName !== 'LINK')
                    Logger.warn("(abpResult = true)   -> "+loc.spec+" / tagName: "+node.tagName);
                    
                return abpResult; // Ci.nsIContentPolicy.ACCEPT;
            }
        }
        catch(e) {
