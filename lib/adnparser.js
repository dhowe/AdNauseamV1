...
const DELETE_ADS_ON_CLOSE = true; // TESTING-ONLY: REMOVE
...
DELETE_ADS_ON_CLOSE && (this.adlookup = {});
	...
	createAd : function(rNode, theUrl, pageUrl, newNode) {
		...
		while (!this.validateLink(link)) {
			...
			if (!/^http/i.test(link)) {
				...
				var absolute = parts[0]+'//'+parts[2];
				var rellink = absolute + link;
				if (link !== rellink) {
					link = rellink;
	...
	handleOther : function(tag, rNode, wnd, node, type, loc, abpResult) { 
		...
		//console.log('handleOther: TYPE_SUBDOCUMENT='+(type === Ci.nsIContentPolicy.TYPE_SUBDOCUMENT));
	 	...
	 	if (tag === 'img') {
			...
			Logger.log("IFRAME-SRC: "+isrc);
			var iid = rNode.getAttribute('id');
			Logger.log("IFRAME-ID: "+iid);
			//Cu.import('resource://gre/modules/Services.jsm');
			logIframeHtml(rNode, 0);
			...
			console.log("b3: "+rNode.contentWindow.document.body.innerHTML);
