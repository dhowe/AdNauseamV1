...
var AbpComponent = require('sdk/core/heritage').Class({
    ...
    processNode : function(wnd, node, type, loc, collapse) { 
        ...
        if (!node.ownerDocument && !abpResult && !isHidden)  {
            ...
            if (verbose) Logger.log("OVERRIDE+ALLOW(!ownerDocument) -> type="+type+" hidden="+isHidden +
                ', abpResult='+abpResult+', selector='+loc.selector+'\n\t\t\t'+loc.spec);
                
            return Ci.nsIContentPolicy.ACCEPT;
        }
        
if (!abpResult) { // tmp: allow-scripts
    
    if (type == 2) {
        console.log("** NO-BLOCK",loc.spec,abpResult);
        return true;
    }
    console.log("BLOCK",loc.spec,abpResult);
