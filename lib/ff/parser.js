...
//console.log("handleHiddenElement\n",this.elemHideMap[url]);
...
console.log("ATTACH-TAB-WORKER: "+url);
    	        ...
    	        tabWorker = require('sdk/tabs').activeTab.attach({
                ...
                tabWorker.port.on("parsed-text-ads", function(ads) {
                    
                    //console.log("Parser.on('parsed-text-ads') -> "+ads.length);
                    
                    for (var i=0; i < ads.length; i++) {
                     
                        me.processTextAd(ads[i]);
                    }
                });
    ...
    handleAd : function(wnd, node, type, loc, collapse, abpResult) {
        ...
        var parentUrl = wnd.wrappedJSObject.document.URL, 
            ...
            currentUrl = uiMan.currentPage();

if (0 && parentUrl !== currentUrl)  // only on iframes?
        ...
        if (tag === 'a') {  // TODO: DEAL WITH OTHER TAGS
        	...
        	if (!this.ignoreRE.test(link)) {
	        	...
	        	var ad = this.createAd(rNode, loc.spec/*image-url*/, currentUrl/* page-url*/);
    ...
    processTextAd : function(ad) {
        
        //console.log('processTextAd1',ad, ad.site+"/"+ad.text);
        ...
        var adObj = { // Ad-Object
            ...
            title : ad.title,  
            contentType : 'text', 
            hashKey : ad.title + '/' + ad.site + '/' + ad.text,
        ...
        this.processAd(adObj, adObj.pageUrl);
    },
