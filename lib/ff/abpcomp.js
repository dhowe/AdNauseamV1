...
var AbpComponent = require('sdk/core/heritage').Class({
    ...
    processNode : function(wnd, node, type, loc, collapse) { 
        ...
        var parentUrl = wnd.wrappedJSObject.document.URL, isHidden = (type == elemHide), 
        ...
        if (isHidden) {  // keep track of hidden elements  
            
            // can pass loc.selector, node, wnd, parentUrl
            parser.handleHiddenElement(parentUrl, loc.selector);
        }
        
        var ownerDocument = node.ownerDocument;
    
        // ignore pages loaded in adn-tab, vault/menu
        if (parser.isAdnWorker(wnd) || parser.ignoreRE.test(parentUrl))
            return true;
...
///////////////////////////////////////// PROBLEM /////////////////////////////////////////
        if (!ownerDocument) {
            ...
            if (0) Logger.log("IGNORE(no ownerDocument) -> " + loc.spec + "\n" + parentUrl);
            return abpResult; 
        }
//////////////////////////////////////////////////////////////////////////////////////////
// if return true here, google-text-ads not blocked, if return abpResult, no ads parsed at all (nytimes) 
//////////////////////////////////////////////////////////////////////////////////////////  
     

        if (abpResult || !Options.get('enabled'))
            return abpResult;
