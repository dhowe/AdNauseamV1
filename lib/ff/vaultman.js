  ...
  onAdAttempt: function(ad) {
    ...
    if (tab && this.worker) { // is vault open?
      ...
      if (tab === require('sdk/tabs').activeTab) { // active
        ...
        this.worker.port.emit("set-current", {
          current: ad
        });
      }
    }
  ...
  onAdFound: function() {
    ...
    if (tab) {
      ...
      if (this.worker) { // is vault open?
        ...
        require("./logger").Logger.warn('Unexpected state(1): vault open with null worker!');
      ...
      this.worker = tab.attach({
        ...
        contentScriptOptions: {
          ...
          'locale': {
            ...
            //ad: locale("adn.inspector.ad"),
            target: locale("adn.inspector.target"),
            foundOn: locale("adn.inspector.foundOn"),
        ...
        contentScriptFile: [
          ...
          Data.url('lib/jquery.mousewheel.min.js'),
        ...
        if (json.ids) {
          ...
          for (var i = 0, len = json.ids.length; i < len; i++) {
            ...
            if (!parser.deleteAd(json.ids[i])) {
              ...
              require("./logger").Logger.warn('Unexpected state(3): could not delete ad#' + json.ids[i]);
            }

            require("./adnutil").AdnUtil.log("VAULT: deleted-id: " + json.ids[i]);
          }
        }
      });
