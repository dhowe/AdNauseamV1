const verbose = 0, pollQueueInterval = 5000, disabled = false;
...
var AdVisitor = require('sdk/core/heritage').Class({
    ...
    beforeLoad : function(channel, subject, url, origUrl) {
        ...
        //  (fix for #12) TODO: this is really not adequate 
    ...
    pollQueue : function() {
        ...
        if (!Options.get('enabled') || disabled) {
            ...
            console.log('**** [WARN] visitor.disable = true ****');
    ...
    checkNext : function() {
        ...
        if (VaultMan.inspected && VaultMan.inspected.visitedTs === 0) {
            ...
            console.log('checkNext pushing inspected ad: '+VaultMan.inspected.id);
    ...
    restart : function() {
        ...
        this.pageWorker = Page({  // can we re-use this?
            ...
            allow: { "script": this.allowJsOnPage } // allow or disallow js from loaded url
        }); 
        
        this.pageWorker.view.setAttribute("ADN", "ADN");
        this.pageWorker.port.on("page-visited", this.pageVisited.bind(this));
        this.pageWorker.on("DOMContentLoaded", function() { console.log("DOMContentLoaded") } );
        this.pollQueue();       
    },
