...
var UIManager = require('sdk/core/heritage').Class({
        ...
        initialize: function() {
            ...
            //this.auto = !!require('sdk/system/environment').env.ADN_AUTO || false;
            //this.cfx && (this.auto = require('sdk/system/environment').env.ADN_AUTO);

            //console.log("DEV: ", this.cfx);//require('sdk/system/environment').env.ADN_DEV, require('sdk/system/environment').env.ADN_AUTO, this.auto);
        ...
        startCountInserter: function() {
            ...
            require("sdk/page-mod").PageMod({
                    ...
                    contentScriptFile: ['./lib/jquery-1.11.2.min.js', './count-inserter.js'],
                    ...
                    onAttach: function(worker) {
                        ...
                        worker.on('detach', function() { me.detachWorker(this, me.workers); });
                    }
                });
        },
            ...
            if (count && workers) {
                ...
                for (var i=0, len = workers.length; i<len; i++) {
                    if (workers[i].url === pageUrl && workers[i].port) {
                        workers[i].port.emit('insert-count',{ count: count });//, automated: auto });
                    }
                }
            }
        },
        ...
        updateMenu: function(update) {
            ...
            if (this.menuIsOpen()) {
                ...
                if (update) {
                    ...
                    if (!menuAds.count && parser.adlist.length) // no page-ads, only recent
                    ...
                    this.menu.port.emit("layout-ads", {
                            ...
                            emptyMessage: msgNoAds // localized
        ...
        buttonIcon: function(size, pressed) {
            ...
            (pressed ? Data.url('img/icon-v-' + size + '.png') : Data.url('img/icon-' + size + '.png')) : Data.url('img/icon-g-' + size + '.png');
        },
