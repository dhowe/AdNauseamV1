...
Cu.import("resource://gre/modules/NetUtil.jsm"); // not-used
...
var AdParser =  require('sdk/core/heritage').Class({
    ...
    getOriginWindow: function(/**Window*/ wnd)   // not-used
    {
        while (wnd != wnd.parent) {
            var uri = this.makeURI(wnd.location.href),
                nutil = Cc["@mozilla.org/network/util;1"]
                    .getService(Ci.nsINetUtil);
                
            if (uri.spec != "about:blank" && uri.spec != "moz-safe-about:blank" && 
                !nutil.URIChainHasFlags(uri, 
     ...
     * invalid URIs.
     */ 
    makeURI : function(/**String*/ url) /**nsIURI*/ { // not-used
        try {
    ...
    handleAd : function(wnd, node, type, loc, collapse, abpResult) {
        ...
        var uiMan = require("./uiman").UIManager,
            parentUrl =  wnd.top.location.href,
            currentTitle = wnd.top.document.title;
            //originWindow = this.getOriginWindow(wnd),
            //wndLocation = originWindow.location.href,
            //parentUrl = wnd.wrappedJSObject.document.URL, // TODO: incorrect: shows iframe instead of parent?
            //currentTitle =  wnd.wrappedJSObject.document.title; // is this also wrong in some cases?
            
            
        //if (wnd.top.location.href != parentUrl)Logger.warn("ALERT **** "+parentUrl +' != '+wnd.top.location.href);
...
//Logger.log("pre-find: "+node.tagName+' :: '+loc.spec);
		...
		var link = 'unknown', tag = rNode.tagName;

		//Logger.log("post-find: "+tag+' :: '+loc.spec+' :: '+type);//+' '+newNode._width+'x'+newNode._height);
	    
	    if (tag !== 'A' && tag !== 'IMG') {
	        
            //Util.log("FOUND: "+tag, loc.spec);
	        rNode = this.walkDOM(rNode, 'A');	        
            if (rNode) tag = rNode.tagName;
            //Util.log("PARSED: "+(tag==='A'));
	    }
	    
        if (tag === 'A') {

            this.handleAnchor(rNode, loc, parentUrl, currentTitle);
        }
       	else {
       		
       		if (1 && tag !== 'IMG' && tag !== 'LINK')
        	   Logger.log('UNHANDLED('+tag+"): "+(loc ? loc.spec : 'no url!'));
       	}
       	
        return abpResult; 
	},
	
    walkDOM : function(n, tag, log) {
        
        log = log || n.tagName == 'IFRAME';
        //log && Util.log("walkDOM:"+tag);
        do {
            ...
            if (n.hasChildNodes())    
                return this.walkDOM(n.firstChild, tag, log);
	...
	searchDOM : function(node, tag) { // ugly, but faster?
	    ...
	    //console.log("walkDom: "+node.tagName);
        var ch = node;
        while (ch) {
    ...
    handleAnchor : function(rNode, loc, parentUrl, currentTitle) {

//Util.log('handleAnchor('+parentUrl+'): '+rNode.tagName,loc.spec);

        link = rNode.getAttribute("href") || 'unknown';
    ...
    makeAd : function(adId, pageTitle, pageUrl, targetUrl, contentData, adTitle) {
        ...
        if (ad.contentType === 'text')
            ...
            Logger.log("FOUND-AD: #"+ad.id+"/"+this.adlist.length+" "+ad.contentData.src+" page: "+pageUrl);
