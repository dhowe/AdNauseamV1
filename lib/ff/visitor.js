...
var AdVisitor = require('sdk/core/heritage').Class({
  ...
  pollQueue: function() {
    ...
    if (!Options.get('enabled') || disabled) {
      ...
      Util.warn('Visitor.disabled = true ***');
    ...
    if (verbose) {
      ...
      Logger.log("AdVisitor.pollQueue :: found " + (next ? 'ad#' +
        next.id : 'NULL AD (none left?)') + ' total=' + this.parser.adlist.length);
    }
    ...
    if (next) {
      ...
      if (this.currentAd === next) { // still not visited...
        ...
        Logger.log("TIMEOUT: (no visit) -> #" + this.currentAd.id + " :: " +
          this.currentAd.targetUrl + " " + (this.currentAd.attempts + 1) + "/" + this.maxAttempts);

        var forceFail = require('../config').PARSER_TEST_MODE === 'update';
        if (forceFail) Util.warn("FORCE FAIL ******* ");
        ...
        if (next.targetUrl) {
          ...
          if (/^http/.test(next.targetUrl)) {
            ...
            try {
              ...
              Util.error('Visitor: unable to visit -> ' +
                next.targetUrl, e);
            }
          }
          else {
