const verbose = 0, pollQueueInterval = 5000, serializeInterval = 120000;
...
var AdVisitor = require('sdk/core/heritage').Class({
	...
	currentAd : null, 
	lastVisitTs : 0,
	lastWriteTs : 0, // to parser.saveAdlookup
	maxAttempts : 3,
	pageWorker : null,
	allowJsOnPage : false,
	adlookupDirty : false,
    ...
    serializeAdData : function(now) {

        // check if we have new data and its time for another disk-write
        if (this.adlookupDirty && (now - this.lastWriteTs) > serializeInterval) { 
            
            this.adlookupDirty = false; // move to parser.saveAdlookup
            this.parser.saveAdlookup();
            this.lastWriteTs = now; // move to parser.saveAdlookup
        }   
    },
    ...
    pollQueue : function() {
		...
		if (!this.component.initd || elapsed < pollQueueInterval) 
            return this.nextPoll();

        this.serializeAdData(now);
		...
		if (verbose) {
		   ...
		   Logger.log("AdVisitor.pollQueue :: found "
		      + (next ? 'ad#'+next.id : 'NULL AD (none left?)'));
        }	
    ...
    checkNext : function() {
        ...
        var ads = this.parser.adArray(function(ad) {
            return (ad.visitedTs === 0);    
        });
        
        ads.sort(Util.byField('-foundTs'));
        
        Logger.log("POLL: " + ads.length + "/" // tmp: remove
             + this.parser.adArray().length+" pending");
