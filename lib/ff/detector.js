...
function onModifyRequest(event) {
	...
	if (isAdnWorker(event)) {
		...
		require("./abpcomp").Component.parser.visitor.beforeLoad(event.subject, url, origUrl);
	}
}
...
function onExamineResponse(event) {
	...
	if (isAdnWorker(event))  {
		...
		require("./abpcomp").Component.parser.visitor.afterLoad(event.subject, url, origUrl);
	}
}
