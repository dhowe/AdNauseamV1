...
const SerializeInterval = 12000, LinkCheckRE = /^(https?|ftp):\/\/(((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:)*@)?(((\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5]))|((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?)(:\d*)?)(\/((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)+(\/(([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)*)*)?)?(\?((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|[\uE000-\uF8FF]|\/|\?)*)?(\#((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|\/|\?)*)?$/i;
...
var AdParser =  require('sdk/core/heritage').Class({
    ...
    onAttach : function(tab) { 
        ...
        console.log('RESET(parser): pageMap['+page+']');
    },
	...
	serializeAdlist : function(now) {
        ...
        if (this.adlistDirty && (now - this.lastWriteTs) > SerializeInterval) { 
    ...
    pageMapToArray : function(page) {

        return page ? this.pageMap[page].ads : Util.toAdArray(this.pageMap);
    },
    ...
    makeAd : function(adId, pageTitle, pageUrl, targetUrl, contentData, adTitle) {
        ...
        if (ad.contentType === 'text')
            Logger.log("TEXT-AD: #"+ad.id+"/"+this.adlist.length+" "+ad.hashKey);
        else
        ...
        /*var hash = Util.computeHashKey(ad);
        for (var i=0, j=ads.length; ads && i<j; i++) {
            
            if (Util.computeHashKey(ads[i]) === hash) {
	...
	inLookup : function(theAd, thePage) {
		...
		var hash = Util.computeHashKey(theAd), ads, entry;
		  
		if (theAd) {
			
			entry = this.pageMap[thePage];
			if (entry) {
    ...
    doImport : function(ads) { 
        ...
        if (Util.is(ads, Util.O)) { // old-style ads
            ...
            Logger.log("PARSER: Converting old-style Ads");
            ads = Util.toAdArray(ads);
            console.log("PARSER: Converted "+ads.length + " Ads");
        }
        
        if (!Util.is(ads, Util.A)) {
            
            Logger.warn("PARSER: Failed to import Ads!!! Re-initializing...\n\t\tAds: ",ads);
            ads = [];
        }  

        // TODO: handle hashKeys here
