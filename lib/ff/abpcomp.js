...
var AbpComponent = require('sdk/core/heritage').Class({
    ...
    processNode : function(wnd, node, type, loc, collapse) { 
        ...
        if (0) console.log(parser.typeStr(type) + (loc.spec||loc.selector||loc));
		...
		if (isScript) {
            ...
            // Allow scripts as they may generate ads we need to handle? (see issue 245)    
            if (parser.allowScriptDomains.indexOf(wnd.top.document.domain) > -1) {
                
                !abpResult && console.warn("*** Allowing ABP-blocked script: "+loc.spec);
                return Ci.nsIContentPolicy.ACCEPT;
            }
            else {
                
                !abpResult && console.warn("*** Blocking script: "+loc.spec);
                return abpResult;
            }
        }
