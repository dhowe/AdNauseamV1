...
var UIManager = require('sdk/core/heritage').Class({
        ...
        openLog: function() {
            ...
            console.log(require("./logger.js").Logger.logJSON);
            
            var tabs = require("sdk/tabs"),
                worker;

            tabs.open({
                url: Data.url("log.html"),
            ...
            function sendLogData(tab) {
                ...
                if (tab && tab.url === Data.url("log.html")) {

                    worker = tabs.attach({

                        contentScriptOptions: {
                            'logJSON': require("./logger.js").Logger.logJSON
                        },
            ...
            worker && worker.port.emit('show-log');
            
/*            // check if Log has been open
            for (var tab of tabs) {
                if ( tab.url.contains( require("./logger").Logger.fileName ) ) {
            ...
            var path = "file://" + require("./logger").Logger.logFile.path;
            tabs.open({
                url: Data.url(path),
                
                // added Event listener to scroll to bottom after page loading
                onPageShow: function scrollToBottom(tab) {
                    tab.attach({
                        contentScript: 'window.scrollTo(0, document.body.scrollHeight);' +
        ...
        cleanupTabs: function() {
            ...
            var tabs = require("sdk/tabs"),
                inDataDir = new RegExp('^' + Data.url());

            for (var tab of tabs) {

                if ( inDataDir.test(tab.url) || tab.url.contains(require("./logger").Logger.fileName) ) {
