...
var UIManager = require('sdk/core/heritage').Class({
  ...
  injectCountOnPage: function(workers) { // count and color
    ...
    prefs["automated.last.count"] = count;
    prefs["automated.last.URL"] = pageUrl;
    ...
    if (count && workers) {
      ...
      for (var i = 0, len = workers.length; i < len; i++) {
        ...
        if (workers[i].url === pageUrl && workers[i].port)
          require("./logger").Logger.log("fire: insert-count->"+count);
  ...
  cleanupTabs: function() {
    ...
    for (var tab of Tabs) {
      ...
      if (inDataDir.test(tab.url) || tab.url.contains("log.html") ||
        tab.url.contains(require("./logger").Logger.fileName)) {
  ...
  registerEventHandlers: function() {
    ...
    this.menu.port.on("disable-logs", function(arg) {
      ...
      require("./logger").Logger.log('UI->disable-logs: ' + value);
    ...
    this.menu.port.on("disable-referer", function(arg) {
      ...
      var value = (arg && arg.value);
      require("./logger").Logger.log('UI->disable-referer: ' + value);
    ...
    this.menu.port.on("clear-ads-with-history", function(arg) {
      ...
      var value = (arg && arg.value);
      require("./logger").Logger.log('UI->clear-ads-with-history: ' + value);
    ...
    this.menu.port.on("hide-badge", function(arg) {
      ...
      var value = (arg && arg.value);
      require("./logger").Logger.log('UI->hide-badge: ' + value);
  ...
  importAds: function(theFile) {
    ...
    try {
      ...
      require("./logger").Logger.error('Error on import', e);
    }
  },
  ...
  exportAds: function() {
    ...
    if (rv == Ci.nsIFilePicker.returnOK || rv == Ci.nsIFilePicker.returnReplace) {
      ...
      promise.then(
        function(aVal) {
          require("./logger").Logger.log('Saved to file');
        },
        function(aReason) {
          require("./logger").Logger.log('writeAtomic failed: ', aReason);
        }
      );
    }
  }
