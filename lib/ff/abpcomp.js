...
var AbpComponent = require('sdk/core/heritage').Class({
    ...
    processNode: function(wnd, node, type, loc, collapse) {
        ...
        if (isScript) {
            ...
            var allowDomain, domain = wnd.top.document.domain;
            
            //console.log("script-domain: "+domain);

            allowDomain = (parser.whitelistedScriptDomains.some(function(re) {
                  
                return (typeof re == 'object') ? re.test(domain) : re === domain;
            }));

            /*  Allow scripts from whitelisted domains as  
                they may generate ads we need to handle  */    
            
            if (allowDomain) {

                if (!abpResult) Util.log("*** Allowing ABP-blocked script: " + loc.spec);

                return Ci.nsIContentPolicy.ACCEPT;
            }
            else  {
            
                if (!abpResult) Util.log("*** Blocking script: " + loc.spec);
        ...
        if (!node.ownerDocument && !abpResult && !isHidden) {
            ...
            if (type != Ci.nsIContentPolicy.TYPE_IMAGE) Util.log('OVERRIDE/ALLOW' +
    ...
    stop: function(reason) {
        ...
        try {
            ...
            var clearHistoryOnExit = require("sdk/preferences/service")
                .get("privacy.sanitize.sanitizeOnShutdown");


            // only want to do this if selected in user prefs
            if (clearHistoryOnExit && require("./options").Options.get("clearAdsWithHistory")) { // TODO: localize
        
                Logger.log('UI->clear-ads-with-history-exit');
                require("./abpcomp").Component.parser.clearAds();
                //require("./logger").Logger.dispose();
            }
