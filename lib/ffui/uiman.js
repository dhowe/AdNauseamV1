...
var UIManager = require('sdk/core/heritage').Class({
    ...
    updateMenu : function(update) { 
        ...
        if (this.menuOpen()) {
            ...
            if (update) {
                ...
                //console.log('updateMenu.update()', update.length, update);
                this.menu.port.emit("update-ads", { 
                    
                    data: this.toAdArray(adhash), 
                    currentAd: current,
                    update: update, 
                    page: pageUrl
                ...
                console.log('updateMenu.layout(): '+pageUrl, Object.keys(adhash));
    ...
    // CHANGED(12/19): Each ad is now visited separately
    // this function should now take a single ad
	updateVault : function(update) {

        console.log("UIMan.updateVault: "+update);

    // TODO: should check tab passed as arg
        ...
        this.worker || (this.worker = this.tab.attach({
			...
			onReady: //this.updateVaultGuts.bind(this)
			     function(data) {                 console.log("UIMan.updateVault.onReady"); }
			
		}));

        this.worker.port.on("ready", function(data) { // from vault.js

            //require('sdk/tabs').activeTab.close();
                console.log("UIMan.updateVault.ready");
    ...
    updateVaultGuts : function(update) {

        console.log("UIMan.updateVaultGuts: "+update);
...
//        var adSets = AdSet.createAdSets(adhash);
//console.log("ADSETS: "+adSets.length);

		if (update) { 
			
			console.log("UIMan.updateVault->update-ad: "+update.id);
			
			this.worker.port.emit("update-ad", {  // -> vault.js
			...
			console.log("UIMan.updateVault->layout-ads");
		...
		if (!this.tab) {
			...
			function refreshView() {
				...
				if (this.tab.url === Data.url('vault.html')) 
					this.updateVault.apply(this); // TODO: pass tab
					
				if (this.tab.url === Data.url('log.html'))
					this.refreshLogView.bind(this); // TODO: pass tab
			}
