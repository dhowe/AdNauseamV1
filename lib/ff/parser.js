...
// ===========================================================================
	...
	handleHiddenElement : function(url, selector) {
	    ...
	    if (this.handleTextAds) {
    	    ...
    	    if (!this.elemHideMap[url]) { // first hidden element for page
    	        ...
    	        var me = this, activeTab = require('sdk/tabs').activeTab; // problem
                ...
                tabWorker.port.on("parsed-text-ad", function(ad) {
                    ...
                    var adObj = new Ad(++me.count, activeTab.title, activeTab.url, ad.targetUrl, 
    ...
    handleAd : function(wnd, node, type, loc, collapse, abpResult) {
        ...
        //if (parentUrl !== currentUrl)  // only on iframes?console.log('IFRAME: "'+parentUrl+ "\n\tON "+currentUrl+"\n\tON "+loc.spec);
        ...
        if (tag === 'a') {  // TODO: DEAL WITH OTHER TAGS
        	...
        	if (!this.ignoreRE.test(link)) {
	        	...
	        	var ad = this.createAd(rNode, loc.spec/*image-url*/, parentUrl/* page-url*/, currentTitle);
	...
	createAd : function(rNode, theUrl, pageUrl, pageTitle) {
		...
		return new Ad(++this.count, pageTitle, pageUrl, link, { src: theUrl });
	},
...
function Ad(adId, pageTitle, pageUrl, targetUrl, contentData, adTitle) {
    ...
    console.log('AD: ', JSON.stringify(contentData));
