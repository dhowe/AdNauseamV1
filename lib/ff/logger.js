...
// TODO: replace all streams with File.IO
var Logger =  require('sdk/core/heritage').Class({
	...
	dispose : function() { // never called?
		...
		if (this.logFile && this.logFile.exists()) {
			...
			this.logFile.remove();
			if (this.logFile && this.logFile.exists())
				require("./adnutil").AdnUtil.error("Logger.dispose() :: log still exists? "+this.logFile.exists());
			this.logFile = null;
		}
	...
	log : function(msg) { 
		...
		{ 
			...
			if (typeof this.ostream === 'undefined' || !this.ostream) {
				...
				require("./adnutil").AdnUtil.warn('Logger: Unable to create iostream: '+this.logFile.path);
				require("./adnutil").AdnUtil.trace();
		...
		require("./adnutil").AdnUtil.log(msg);
		...
		if (!this.borked && !logsDisabled) {
			...
			if ( typeof this.ostream === 'undefined' || !this.ostream) {
				...
				require("./adnutil").AdnUtil.error("NO IO!\n  Msg: "+msg);
				return;
			}
	...
	warn : function(msg, notify) { 
   	...
   	err : function(msg, e) { 
   		...
   		require("./adnutil").AdnUtil.error(msg);
