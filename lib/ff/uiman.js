...
var UIManager = require('sdk/core/heritage').Class({
	...
	updateOnAdVisit : function(update) {
        ...
        require("./abpcomp").Component.parser.adlookupDirty = true;
    ...
    getMenuAds : function(parser, pageUrl) {
        ...
        var pageMap = require("./abpcomp").Component.parser.pageMap[pageUrl],
            ads = pageMap && pageMap.ads; 

//console.log('getMenuAds('+pageUrl+')');
        var pageCount = ads ? ads.length : 0;
//console.log('getMenuAds: '+pageCount);
        var res = pageCount ? ads : this.recentAds(parser);
        
//console.log('return.getMenuAds('+pageUrl+') -> ', res.length);
        
        return { ads: res, count: pageCount }; 
    },
     
    recentAds : function(parser) {

        var recent = [], ads = parser.adlist, num = 5;
    
        if (ads) {
    
            ads.sort(require('./adnutil').AdnUtil.byField('-foundTs')); // sort by found-time
        ...
        //console.log('recentAds() :: '+recent.length);
...
if (tm) {
    ...
    importAds : function(theFile) {
        ...
        var picker, inputFile, inputStream, fileStream, tmp,
        ...
        parser.doImport(JSON.parse(fileStream));
        parser.logStats();
    },
