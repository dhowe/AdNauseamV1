...
var Logger =  require('sdk/core/heritage').Class({
	...
	writeLogJSONAndSendUpdate: function (msg) {
		...
		// Use Queue to limit the number of entries to the logSize 
		if (this.logJSON.length >= this.logSize)
			this.logJSON.shift();
			
		// store the entries in memory
		this.logJSON.push({ i: this.logJSON.length, msg: msg });
		
		// send update to log.html
		this.updatelogPage();
	},

	updatelogPage : function() {
		
		var UIManager = require("./uiman.js").UIManager;
		if ( UIManager.logHTMLWorker !== null )
			UIManager.updateLogPageFromLogger();
	},
