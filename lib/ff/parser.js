...
var AdParser = require('sdk/core/heritage').Class({
  ...
  blockablePageDomains: [], //'www.webpronews.com', 'www.tomshardware.com', 'www.zdnet.com', 'www.techrepublic.com'],

  // always block scripts from these domains (either regex or string ok)
  blockableScriptDomains: [new RegExp('partner.googleadservices.com')],
  ...
  initialize: function(comp) {
    ...
    this.count = Math.max(0, 1 + (Math.max.apply(Math,
      this.adlist.map(function(ad) {
        return ad.id;
      }))));
  ...
  handleScript: function(wnd, loc, abpResult) {
    ...
    var domain = wnd.top.document.domain;

    var blockDomain = (this.blockablePageDomains.some(function(re) {

      return (typeof re == 'object') ? re.test(domain) : re === domain;
    }));

    var blockScriptDomain = (this.blockableScriptDomains.some(function(re) {

      return (typeof re == 'object') ? re.test(loc.spec) : re === loc.spec;
    }));

    if (!abpResult && (blockDomain || blockScriptDomain)) {

      require('./adnutil').AdnUtil.log("[BLOCK] SCRIPT src=" +
        loc.spec + " (" + domain + ")");
      return abpResult;
    }
  ...
  handleHiddenElement: function(url, selector) {
    ...
    if (!this.elemHideMap[url]) { // first hidden element for page
      ...
      this.tabWorker.port.on("parsed-img-ad", function(ad) {
        ...
        me.makeAd(++me.count, ad.pageTitle, ad.pageUrl, ad.targetUrl, {
          src: ad.imgUrl
        });
      });
  ...
  doImport: function(ads) {
    ...
    if (Util.is(ads, Util.O)) { // old-style ads
      ...
      Logger.log("PARSER: Converted " + ads.length + " ad(s)");
    }
