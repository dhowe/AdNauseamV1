...
// ===========================================================================
    ...
    handleAd : function(wnd, node, type, loc, collapse, abpResult) {
        ...
        //console.log('handleAd :'+type);
		if (type === 65533)
            console.log("[ADN-ELEMHIDE] "+loc.selector);
            ...
            if (newNode) {
		        ...
		        if (tag === 'a') {  // DEAL WITH OTHER TAGS
		        	...
		        	if (!this.ignoreRE.test(link)) {
			        	...
			        	var ad = this.createAd(rNode, loc.spec/*image-url*/, currentTab/* page-url*/, newNode);
			        	
			        	if (!ad) {
			        	    
			        	    Logger.warn("Parser.createAd() fail: "+rNode+','+loc.spec+','+parentUrl+','+newNode);
			        	    return abpResult;
			        	}
	...
	createAd : function(rNode, theUrl, pageUrl, newNode) {
		...
		var tag = rNode.tagName.toLowerCase(), 
            link = rNode.getAttribute("href"), 
            tries = 0;
