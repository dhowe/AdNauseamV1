...
var _AdParser =  require('sdk/core/heritage').Class({
	...
	initialize : function(comp) {
		...
		this.setAds(require("sdk/simple-storage").storage.adlookup);
	...
	setAds : function(ads) {
		...
		Logger.log('AdParser: '+this.getAds().length+' ads ('+
			this.pendingAds().length+' pending, '+this.erroredAds().length+' errors)');
	},
...
//                 	
	...
	createAd : function(rNode, theUrl, pageUrl, newNode) {
		...
		return { // Ad-Object
			...
			hidden : false,
			contentType : 'img', 
			contentData : theUrl, // img-src tag in most cases
